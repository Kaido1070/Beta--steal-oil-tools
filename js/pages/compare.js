...
const compareState={tier:1,area:1,mole:0,fruit:0,rebirth:0,countA:1,countB:1,hours:5,likes:0};
const compareProductionCore=window.STOT_LAYOUT_PRODUCTION;
function compareRebirthLevel(){return compareProductionCore?.rebirthLevel?compareProductionCore.rebirthLevel(compareState.rebirth):Math.max(0,Math.min(50,Math.trunc(Number(compareState.rebirth)||0)))}
function compareRebirthBonus(){return compareProductionCore?.rebirthBonusPercent?compareProductionCore.rebirthBonusPercent(compareRebirthLevel()):compareRebirthLevel()*10}
function compareRebirthMultiplier(){return compareProductionCore?.rebirthMultiplier?compareProductionCore.rebirthMultiplier(compareRebirthLevel()):1+(compareRebirthBonus()/100)}
function ensureCompareRebirthControl(){
  if($("#compareRebirth"))return;
  const grid=$("#compareView .compare-setup-grid");if(!grid)return;
  const field=document.createElement("label");field.className="compare-control";field.id="compareRebirthField";
  field.innerHTML='<span>Rebirth Level <small id="compareRebirthBonus">+0% Production</small></span><input id="compareRebirth" type="number" min="0" max="50" step="1" value="0" inputmode="numeric" placeholder="0 - 50">';
  grid.appendChild(field);
}
function syncCompareRebirthUI(){ensureCompareRebirthControl();const el=$("#compareRebirth"),bonus=$("#compareRebirthBonus");const level=compareRebirthLevel();compareState.rebirth=level;if(el&&String(el.value)!==String(level))el.value=level;if(bonus)bonus.textContent=`+${compareRebirthBonus()}% Production`;return level}
function bindCompareRebirth(){ensureCompareRebirthControl();const el=$("#compareRebirth");if(!el||el.dataset.rebirthBound==='1')return;el.dataset.rebirthBound='1';const update=()=>{compareState.rebirth=compareProductionCore?.rebirthLevel?compareProductionCore.rebirthLevel(el.value):Math.max(0,Math.min(50,Math.trunc(Number(el.value)||0)));syncCompareRebirthUI();renderCompare()};el.addEventListener("input",update);el.addEventListener("blur",update)}
...
function compareStats(d,side){
  ...
  const setup=compareState.tier*compareState.area*count*comparePetMultiplier(d)*compareRebirthMultiplier();
...
function renderCompare(){
  syncCompareRebirthUI();bindCompareRebirth();
