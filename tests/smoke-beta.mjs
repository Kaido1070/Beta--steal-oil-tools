...
assert.equal(await page.locator('meta[name="stot-local-version"]').getAttribute('content'), '5.77');
assert.equal(await page.locator('html').getAttribute('data-stot-beta-ready'), '5.77');
assert.equal(await page.locator('html').getAttribute('data-stot-database-page'), '5.77');
assert.equal(await page.locator('html').getAttribute('data-stot-events-page'), '5.77');
assert.equal(await page.locator('html').getAttribute('data-stot-codes-page'), '5.77');
assert.equal(await page.locator('html').getAttribute('data-stot-compare-page'), '5.77');
assert.equal(await page.locator('html').getAttribute('data-stot-drills-page'), '5.77');
assert.equal(await page.locator('html').getAttribute('data-stot-sale-page'), '5.77');
assert.equal(await page.locator('html').getAttribute('data-stot-oil-page'), '5.77');
...
assert.deepEqual(configSnapshot, {
  ...
  oilBadge: 'Oil Layout',
  storageKey: 'stot-v5.77-layout-save-v1',
});
...
assert.equal(changelogSnapshot.latestVersion, '5.77');
...
assert.deepEqual(scriptSrcs, ['/js/site-config.js', '/js/changelog.js', '/js/game-data.js', '/js/app.js', '/js/pages/sale.js', '/js/pages/oil.js', '/js/pages/oil-visual-builder.js', '/js/pages/drills.js', '/js/pages/compare.js', '/js/pages/database.js', '/js/pages/events.js', '/js/pages/codes.js', '/js/beta-patches.bundle.js']);
const styleHrefs = await page.locator('link[rel="stylesheet"]').evaluateAll(nodes => nodes.map(n => new URL(n.href).pathname));
assert.deepEqual(styleHrefs, ['/css/app.bundle.css', '/css/pages/oil-visual-builder.css', '/css/pages/drills.css', '/css/pages/compare.css', '/css/pages/database.css', '/css/pages/events.css', '/css/pages/codes.css']);
...
assert.equal(await page.locator('html').getAttribute('data-stot-visual-builder'), '5.77');
...
console.log('SMOKE PASS: v5.72 Central config + separated page modules, pet images, calculators, Preset UI, filters, reload');
await browser.close();